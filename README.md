...
The overlay on `https://www.strava.com/maps/create/*` reads **`GET /api/trails`** and **`GET /api/networks`** from whatever origin you set in the **extension popup** (stored in `chrome.storage.sync` as `apiUrl`). The main-world script does **not** read page `localStorage` for the API base; it asks the isolated [`content-bridge.js`](browser-extension/content-bridge.js), so the console line **`[TrailOverlay] Using API:`** matches the URL used for fetches. Main-world and bridge communicate with `window.postMessage` using a private envelope (`__trailOverlayToBridge` / `__trailOverlayFromBridge`) so Strava’s own `message` traffic cannot be mistaken for overlay responses.

- Default popup URL is `http://localhost:3000` (no trailing slash). Point it at your local Next origin (any port) or at production; [`manifest.json`](browser-extension/manifest.json) grants `http://localhost/*` and `http://127.0.0.1/*` for local fetches.
- The popup **Map layers** section has independent checkboxes for **Trails**, **Networks**, and **List thumbnails** (`overlayTrailsVisible` / `overlayNetworksVisible` / `overlayTrailPhotosVisible` in `chrome.storage.sync`). **Bookmark outline on map** sets `overlayBookmarkHighlightColor` in `chrome.storage.sync`: built-in **yellow**, **red**, **blue**, or **green**, or **Custom** with a **`#rgb` / `#rrggbb`** value; the Strava map applies it to the wide bookmark halo line (opacity is fixed in the extension). **List thumbnails** controls whether the extension fetches community **`GET /api/trail-photos`** data for the current map bounds to populate **small previews on the in-map trail list** (no photo markers on the map). The signed-in web map can also use **`GET /api/trail-photos/mine`**; that route requires your app session, so the extension does not use it on `strava.com`. The **Trail Overlay** master switch under Strava **Map display** toggles `overlayEnabled`; when it is off, trails, networks, and thumbnail fetching stay off regardless of the popup.
- When **Trails** are on, [`browser-extension/content.js`](browser-extension/content.js) shows one **bottom-right panel** inside the Mapbox map container (`#trail-overlay-sonner-panel`): **bookmark cards** sit at the **bottom** of the map box in an overlapping **Safari-style tab** stack (negative vertical overlap so only a **top sliver** of the card behind shows; scroll the stack to peel older cards **down** off screen). A **divider** separates that from the **Trails in view** block: **Bookmark map trail** quick row, **+ N more** (when over 200 viewport trails), a **top-faded vertical scroll** of Sonner-style rows that **grow upward from the bottom** toward Strava’s top chrome, and the section label at the top. **Row body click** (not the star) opens the **large centered detail modal**; **stars** on list rows, the quick row, the modal header, and bookmark cards toggle bookmarks. When **List thumbnails** is on, list previews use the latest viewport **`GET /api/trail-photos`** bbox response (matching `trailId` with normalized ids); when a trail has **no** preview, the list and bookmark cards show **ski-style difficulty icons** (green circle, blue square, black diamond, double black diamond; `not_set` uses a muted “?” badge). The detail **photo grid** loads **`GET /api/trail-photos?trailId=…`** and **merges in** any pins from that same bbox response so the gallery always includes what the list could see, even if the trail-id query path disagrees. Hovering or focusing a list row highlights that line on the map (Mapbox feature state `highlight`). Bookmarked lines show a **faint yellow outline** (wide halo under the line, feature state `bookmarked`). Bookmark ids persist in **`chrome.storage.local`** as `trailBookmarkIds` (through [`content-bridge.js`](browser-extension/content-bridge.js)); changes sync across Strava tabs via `chrome.storage.onChanged`. The modal uses a dim **backdrop** (click outside to close), up to **720px** wide, with height reduced when the panel is visible so content stays above it — **not** a Mapbox `Popup`. **×**, backdrop click, or **Escape** closes it and clears the line highlight. The panel hides when the overlay master switch is off, or when there are no viewport trails **and** no bookmarks while trails are drawn.
- Layer pref updates, `style.load`, and viewport thumbnail refetches **reuse** existing Mapbox trail sources (via `setData` when possible) and **diff** the trail list so the trail detail modal is not closed and the list does not rebuild on every `moveend` when the visible trail set is unchanged.
- Changing **API URL** in the popup notifies the Strava tab so trail/network caches are cleared and data is refetched without a full page reload.
- After editing extension files or `manifest.json`, use **Reload** on the extension in `chrome://extensions`.
