...
- Default popup URL is `http://localhost:3000` (no trailing slash). Point it at your local Next origin (any port) or at production; [`manifest.json`](browser-extension/manifest.json) grants `http://localhost/*` and `http://127.0.0.1/*` for local fetches. For production `https://…` origins, the popup requests the specific host permission when you click **Save**.
